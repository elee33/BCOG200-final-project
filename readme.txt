...
Documentation/Outline of our game: We will be creating a wordle game that tests the user's knowlwedge of BCOG related terms that we have learned throughout this course and previous courses. Wordle is not only a popular game that many people play daily, but it is stimulatory for the brain and a good exercise. Our wordle game models the classic wordle from the New York Times, with the user having to type in a 5 letter word and receiving different colors (red, green, yellow) representing incorrect, correct, and misplaced letters. Our input data in the code will include a txt file of 5 letter BCOG related terms. In the code to guess a word, we will use a for function that includes an if guess to print colored letters green, and elif guess to print colored letters yellow and then 'else' to print the guess. If input word equals the correct word, then "Congrats! You got the wordle in {a certain amount of words}" will print in red. There will then be a phrase "want to play again?" or "Type q to exit the game". 
